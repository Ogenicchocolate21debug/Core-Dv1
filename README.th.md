...
แล้วมันจะเดินไล่เครือข่ายทีละ hop อ่านสุขภาพทุกตัวที่เจอ วาดผัง แล้วออกรายงานที่ส่งมอบเจ้าของไซต์ได้เลย
...
`/netwalk-login` เปิดหน้าเว็บใช้ครั้งเดียวบนเครื่องคุณเอง คุณพิมพ์รหัสลงในเบราว์เซอร์ มันถูกเขียนลงไฟล์ส่วนตัว
...
**สิ่งที่มันกันไม่ได้ พูดตรง ๆ:** process อื่นที่รันในชื่อ user เดียวกันบนเครื่องคุณเอง เข้า loopback ได้
ซึ่งเป็น trust boundary เดียวกับไฟล์ `0600` อยู่แล้ว — ถ้าบัญชีคุณถูกเจาะ ฟอร์มนี้ไม่ใช่สิ่งที่ช่วยคุณ

### 3. มันไม่ sweep ช่วง IP ที่ไม่มีใครอนุญาต
...
ที่ครอบมันอยู่) · public address space ถ้าไม่ใส่ flag ที่สองอย่างจงใจ → ปฏิเสธ เพราะการเป็นเจ้าของ IP เดียว
...
มันจะก๊อป runtime ไปที่ `~/.claude/skills/netwalk/toolkit/` แล้วเขียนโฟลเดอร์ skill ทั้ง 6 ตัว
...
netwalk มีสองชั้น: **สคริปต์ Python ล้วนที่ไม่ต้องมี agent เลย** กับชุดคำสั่งที่บอก agent ว่าจะขับมันยังไง
...
**แล้วมันก็ crawl** — LLDP, CDP, MNDP, ARP, DHCP lease, routing table และ MAC table ต่อพอร์ต
...
**แล้วมัน sweep สิ่งที่ crawl มองไม่เห็น** — TCP-connect sweep ในช่วงที่อนุญาต หาเซิร์ฟเวอร์ที่ตั้ง IP นิ่ง
...
ผู้ใช้) จะถูกทำเครื่องหมายไว้ · **มันมองไม่เห็น UDP** และมองไม่เห็น host ที่ drop แทนที่จะ reject
ซึ่งข้อจำกัดนี้ถูกเขียนลงหัวข้อ coverage ของรายงานให้อัตโนมัติ ไม่ปล่อยให้เข้าใจเอาเอง

**แล้วมันตรวจ config เทียบกับ best practice ของ vendor** — ไม่ใช่จากความจำ แต่ checklist เป็น *data*
...
VLAN ผู้ใช้ · **มันอ่าน config export จากดิสก์โดยตรง** config ที่เต็มไปด้วย PSK จึงไม่ผ่านโมเดล
และ check ที่รัน*ไม่ได้*จะถูกรายงานชื่อออกมา ไม่ใช่ถูกนับว่าผ่าน

**แล้วมันอ่าน health** — CPU, memory, storage, อุณหภูมิ, งบ PoE, error และ CRC ของ interface,
...
ซึ่งเอกสารก็ไม่ได้โม้ว่ามันมากกว่านั้น
...
แต่ละ check พก: หลักฐานที่ใช้ตัดสิน, เหตุผลว่าทำไมมันสำคัญ, วิธีแก้ที่ช่างทำตามได้ และ vendor guidance
ที่มันมาจาก

**3 คุณสมบัติที่สำคัญกว่าจำนวน check:**

- **มันอ่าน config จากดิสก์ ไม่ผ่านบทสนทนา** — export อยู่ตรงนั้นอยู่แล้วเพราะ `netwalk_exec.py --out`
...
- **check ที่อ่าน setting เดียวต้องบอกว่ามันอ่านแค่นั้น** — `mt-dns-remote` ยิงเมื่อเจอ
  `allow-remote-requests=yes` ซึ่งบอกว่า resolver *จะตอบ* ไม่ได้บอกว่ามีอะไร*เข้าถึงมันได้*
  · มันจึงรายงานเป็น *suspected* พร้อมคำสั่งให้ไปอ่าน raw firewall ก่อนเอาไปบอกลูกค้า
...
ไฟล์ JSON หนึ่งไฟล์ต่อหนึ่งไซต์ต่อหนึ่งวันที่สแกน คือแหล่งความจริงเดียว · ไดอะแกรมกับรายงาน render จากมัน
...
**ไม่มีอะไรหมดอายุเอง** — credential store ไม่มี TTL, ไม่มีอะไรลบมันตอนสแกนจบ,
...
| **config export** | `~/.netwalk/sites/<site>/configs/` | PSK, SNMP community, password hash แบบ clear text · **ใหญ่กว่าและอ่อนไหวกว่าไฟล์ credential** และไม่มีอะไรอื่นลบมันให้ |
...
- มันอ่านสิ่งที่อุปกรณ์บอก · อุปกรณ์ที่โกหก หรือ CLI ของ vendor ที่ละอะไรไว้ จะให้ภาพที่ไม่ครบ —
...
- โหมด `--public` ซ่อนหัวข้อ **มันไม่ได้ sanitise ข้อความที่คุณเขียนลง finding เอง**
- gate อ่านอย่างเดียวเป็น allowlist ที่แข็งแรง **ไม่ใช่การพิสูจน์เชิงรูปนัย** · มันมี regression test คุม
...
ไปที่ commit ที่อธิบายมันโดยตรง
